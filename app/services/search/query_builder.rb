...
module Search
  ...
  class QueryBuilder
    ...
    def apply_has_selector(value, relation, negated:, conditions: nil)
      ...
      condition = case normalized
                  when 'attachment'
                    attachment_topic_ids = Attachment.joins(:message).select('messages.topic_id').distinct
                    ["topics.id IN (?)", attachment_topic_ids]
                  when 'patch'
                    patch_topic_ids = Attachment.joins(:message)
                      .where("attachments.file_name ILIKE ? OR attachments.file_name ILIKE ?", "%.patch", "%.diff")
                      .select('messages.topic_id').distinct
                    ["topics.id IN (?)", patch_topic_ids]
                  when 'contributor'
                    # Use denormalized count
                    ["topics.contributor_participant_count > 0"]
                  when 'committer'
                    committer_person_ids = ContributorMembership.where(contributor_type: 'committer').select(:person_id)
                    topic_ids = TopicParticipant.where(person_id: committer_person_ids).select(:topic_id).distinct
                    ["topics.id IN (?)", topic_ids]
                  when 'core_team'
                    core_person_ids = ContributorMembership.where(contributor_type: 'core_team').select(:person_id)
                    topic_ids = TopicParticipant.where(person_id: core_person_ids).select(:topic_id).distinct
                    ["topics.id IN (?)", topic_ids]
                  else
      ...
      if negated
        if condition.size == 1
          relation.where.not(Arel.sql(condition.first))
        else
          relation.where.not(*condition)
        end
      else
        if condition.size == 1
          relation.where(Arel.sql(condition.first))
        else
          relation.where(*condition)
        end
      end
    ...
    def sanitize_fts_query(value)
      # Convert the value to a tsquery-safe format
      # Replace common operators with FTS equivalents
      value.to_s
           .gsub(/\s+&\s+/, ' & ')  # Keep explicit &
           .gsub(/\s+\|\s+/, ' | ') # Keep explicit |
           .gsub(/\s+/, ' & ')      # Implicit AND for spaces (only if no explicit operator)
    end
  end
end
